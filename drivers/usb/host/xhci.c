...
#if defined(CONFIG_USB_XHCI_TCC)  || defined(CONFIG_USB_XHCI_HCD_MODULE)
#include "xhci-tcc.c"
#define	PLATFORM_DRIVER		xhci_tcc_driver
#endif
...
int xhci_run(struct usb_hcd *hcd)
{
	...
	struct xhci_hcd *xhci = hcd_to_xhci(hcd);
	//struct pci_dev  *pdev = to_pci_dev(xhci_to_hcd(xhci)->self.controller);
	struct platform_device *pdev = to_platform_device(xhci_to_hcd(xhci)->self.controller);
	int irq;
