...
static void tcc_xhci_idpin_intr_en(void)
{
   PUSBPHYCFG USBPHYCFG = (PUSBPHYCFG)tcc_p2v(HwUSBPHYCFG_BASE);
...
static void tcc_xhci_phy_off(void)//taejin TODO
{

	tca_ckc_setippwdn(PMU_ISOL_USBOP, 1);
}

static void tcc_xhci_phy_ctrl(int on)//taejin TODO
{
...
void tcc_init_usb3host (uint mode) {
    ...
    if (mode)
    {
		printk("XHCI Only USB 2.0 Mode Support\n");
        powerdown_ss();
    	set_mode_usb20_only();
    }
	else
		printk("XHCI USB 3.0 Mode Support\n");
