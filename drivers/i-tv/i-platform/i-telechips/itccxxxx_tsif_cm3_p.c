...
static int tcc_tsif_cm3_init(void)
{
	st_p_tsif *instance = &p_tsif_inst;
	...
	if( demux->tsif_ex_handle.dma_mode == 0) {
		...
		demux->tsif_ex_handle.serial_mode = 0;
	}
...
static void tcc_tsif_cm3_deinit(void)
{
	st_p_tsif *instance = &p_tsif_inst;
...
int tcc_tsif_p_start(void)
{
	...
	if(tcc_tsif_cm3_init() < 0) {
		printk("[%s] tcc_tsif_cm3_init() fail...\n", __func__);
		return -1;
	}
