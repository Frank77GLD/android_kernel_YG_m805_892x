...
static void tcc_mmc_poll_event(unsigned long data)
{
...
#if defined(CONFIG_ARCH_TCC893X) && !defined(CONFIG_BROADCOM_WIFI) && !defined(CONFIG_WIFI_PWR_CTL)
	if (host->card_inserted == 0) {
		...
		if (host->pdata->switch_voltage) {
			...
			clk_set_rate(host->fclk, 45*1000*1000);
			host->peri_clk = clk_get_rate(host->fclk);
			sdctrl->SD0CMDDAT.nREG = 0x00000F0F;
...
{	// ksjung	- vmmv
	...
	if (ios->clock != 0) {
		...
		if (ios->signal_voltage == MMC_SIGNAL_VOLTAGE_180) {
			...
			//sdctrl->SD0CAPREG0.nREG = 0x0CFF9870;
			sdctrl->SD0CAPREG0.nREG = 0x2CFF9870;
...
static void init_mmc_host(struct tcc_mmc_host *host)
{
	...
	switch (host->pdata->slot % 4) {
		case 0:
			...
			sdctrl->SD0CAPREG1.nREG = 0x00000007;
	#else
			//sdctrl->SD0CMDDAT.nREG = 0x00003F3F;
			sdctrl->SD0CMDDAT.nREG = 0x00000F0F;
			//sdctrl->SD0CMDDAT.nREG = 0x00000000;
			//sdctrl->SD0CMDDAT.nREG = 0x0F0F0F0F;
	#endif
#else
			sdctrl->SD0CMDDAT.nREG = 0x3F3F3F3F;
			sdctrl->SD0CAPREG0.nREG = 0x0CFF9870;
