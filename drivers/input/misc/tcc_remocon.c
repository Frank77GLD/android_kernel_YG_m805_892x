...
static unsigned int tca_rem_readcode(char ch)
{
	...
	case STATUS0:
		...
		if(machine_is_tcc9300() || machine_is_tcc9300st() || machine_is_tcc9300cm()
			|| machine_is_tcc8800() || machine_is_tcc8800st()
			|| machine_is_tcc8920() || machine_is_tcc8920st()
			|| machine_is_tcc893x() || machine_is_tcc8930st())
		{
			if (ch == 'S')
			{
				Rem.Stat = STATUS2;
...
{	
...
#if defined(CONFIG_PBUS_DIVIDE_CLOCK) || defined(CONFIG_ARCH_TCC892X)
  #if defined(CONFIG_ARCH_TCC893X)
...
#else
	int key_type; //remocon_timer.expires = jiffies + msecs_to_jiffies(150);//msecs_to_jiffies(125);
  #if defined(CONFIG_ARCH_TCC893X)
	#if 0
	{
		int i=0;
	...
	else if (Rem.Buf != 0 && (pRcu->BDD.nREG & Hw3 || low_bit=='R' /*|| (pRcu->BDR0.nREG == 0x00000000 && pRcu->BDR1.nREG == 0x00000000)*/))
	{
		key_type = REPEAT_KEY;
	}
	else
...
static int remocon_probe(struct platform_device *pdev)
{
	...
	if(machine_is_tcc8900() || machine_is_tcc9200() || machine_is_tcc9201() || machine_is_tcc9200s()) {
		...
		if(IS_ERR(remote_clk)) {
			...
			clk_set_rate(remote_clk, 1024000);
			//clk_set_rate(remote_clk, 32768);
	#else
			clk_set_rate(remote_clk, 24000000);
	#endif
#elif defined(CONFIG_ARCH_TCC892X)
			clk_set_rate(remote_clk, 12000000);
#endif
			dbg("############## %s: remote clk_rate = %u\n", __func__, clk_get_rate(remote_clk));
		}
	}
...
//	pRcu->PBD00.nREG = 0x00000000;
//	pRcu->PBD01.nREG = 0x00000000;
//	pRcu->PBD10.nREG = power_code&0xFFFFFFFF;
//	pRcu->PBD11.nREG = (power_code>>32)&0x00000007;
}
#endif
