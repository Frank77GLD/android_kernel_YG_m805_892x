...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    GTP_DEBUG_FUNC();
    //GTP_ERROR("+++++++++++++lijinhai ++++goodix_ts_work_func");
...
//	dprintk("*******lijinhai *********\n");
    ...
    if (touch_num)
    {
        ...
        for (idx = 0; idx < GTP_MAX_TOUCH; idx++)
        {
            ...
            pos += 5;
			/*add by lijinhai*/
			/*dprintk("*******lijinhai input_y=%d*********\n",input_y);
			if(input_y>0&&input_y<30){
					input_y+=10;
					dprintk("lijinhai input_y=%d",input_y);
				}*/
			/*end add by lijinhai*/
            gtp_touch_down(ts, idx, input_x, input_y, input_w);
        }
...
static void platform_port_setting(void)
{
#if defined(CONFIG_ARCH_TCC893X)
	if(machine_is_tcc893x())
	{
		gtp_int_irq = INT_EINT2;	
		if(system_rev == 0x1000){
			gtp_rst_port = TCC_GPEXT3(11);
			gtp_int_port = TCC_GPB(11);
			gtp_ext_int = EXTINT_GPIOB_11;
		}
		else if(system_rev == 0x2000 || system_rev == 0x3000){
			gtp_rst_port = TCC_GPEXT3(11);
			gtp_int_port = TCC_GPE(13);
			gtp_ext_int = EXTINT_GPIOE_13;
		}
	}
	else if(machine_is_m805_893x())
	{
		gtp_int_irq = INT_EINT2;	
		if(system_rev == 0x5000 || system_rev == 0x5001){
			gtp_rst_port = TCC_GPC(17);
			gtp_int_port = TCC_GPC(26);
			gtp_ext_int = EXTINT_GPIOC_26;
		}
	}
	...
	if(machine_is_tcc8920())
	{
		...
		if(system_rev == 0x1005 || system_rev == 0x1007){
			...
			gtp_rst_port = TCC_GPEXT3(11);
			gtp_int_port = TCC_GPE(17);
			gtp_ext_int = EXTINT_GPIOE_17;
		}
		else if(system_rev == 0x1008){
			gtp_rst_port = TCC_GPEXT3(11);
			gtp_int_port = TCC_GPE(5);
			gtp_ext_int = EXTINT_GPIOE_05;
		}
		else{
			gtp_rst_port = TCC_GPEXT3(11);
			gtp_int_port = TCC_GPG(18);
			gtp_ext_int = EXTINT_GPIOG_18;
		}
	}
#endif
}
