...
#if (1)
    if (tcc_port->clk)
        uart_clk = clk_get_rate(tcc_port->clk);
...
int tcc_uart_disable(int port_num)
{
	...
	pm_runtime_put_sync_suspend(&tcc_uart_dev[port_num]);
...
#if defined(CONFIG_GPS)
            if(gps_k_flag){
#if defined(CONFIG_MACH_TCC9300) || defined(CONFIG_MACH_TCC8800) || defined(CONFIG_MACH_TCC8920) || defined(CONFIG_MACH_TCC893X)
