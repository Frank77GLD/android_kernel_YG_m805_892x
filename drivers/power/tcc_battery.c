...
#define USB_CHARGE_DETECT_USING_VBUS 0
...
int tcc_cable_status_update(int status)
{
	...
	if (machine_is_m801_88() || machine_is_m803() || machine_is_m805_892x() || machine_is_m805_893x()) {
...
static irqreturn_t usb_vbon_interrupt(int irqno, void *param)
{
...
static irqreturn_t usb_vboff_interrupt(int irqno, void *param)
{
//	printk("\x1b[1;31m%s\x1b[0m\n", __func__);
	disable_irq_nosync(irqno);
	mutex_lock(&tcc_batt_info.lock);
...
static tcc_battery_adc_channel(void)
{
	...
	if(machine_is_tcc8800() || machine_is_m801_88() || machine_is_m803() || machine_is_tcc8920() || machine_is_tcc893x()){
		adc_channel = 5;
		ac_channel = 4;
	}
	...
	else if(machine_is_m805_892x() || machine_is_m805_893x()){
...
static int tcc_battery_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(tcc_power_supplies); i++) {
        ...
        #if defined(CONFIG_STB_BOARD_HDB892S) || defined(CONFIG_STB_BOARD_HDB892F) || defined(CONFIG_STB_BOARD_YJ8935T)
        //Do't register usb notifier
        #else
	     #ifdef CONFIG_TCC_DWC_OTG
	     usb_register_notifier(&usb_status_notifier);
	     #else // for USB3.0
		     #if USB_CHARGE_DETECT_USING_VBUS
		     if (request_irq(INT_USBVBON,usb_vbon_interrupt , IRQ_TYPE_EDGE_RISING|IRQF_DISABLED, "tcc_battery_vbon", NULL)) {
				printk(KERN_ERR "tcc_battery.c: Could not allocate ts IRQ_NO !\n");
				err = -EBUSY;
		     }
		 
		     if (request_irq(INT_USBVBOFF,usb_vboff_interrupt , IRQ_TYPE_EDGE_RISING|IRQF_DISABLED, "tcc_battery_vboff", NULL)) {
				printk(KERN_ERR "tcc_battery.c: Could not allocate ts IRQ_NO !\n");
				err = -EBUSY;
		     }
		     #endif
