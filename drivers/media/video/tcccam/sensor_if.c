...
static enum image_size sensor_find_size(unsigned int width, unsigned int height)
{
	...
	//CONFIG_COBY_MID9125
	#if defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150) || defined(CONFIG_VIDEO_ATV_SENSOR_RDA5888) || defined(CONFIG_VIDEO_CAMERA_SENSOR_HI704) \
	|| defined(CONFIG_VIDEO_CAMERA_SENSOR_OV7690) || defined(CONFIG_VIDEO_CAMERA_SENSOR_SIV100B) || defined(CONFIG_VIDEO_CAMERA_SENSOR_GC0329)\
	|| defined(CONFIG_VIDEO_CAMERA_SENSOR_SP0A18) || defined(CONFIG_VIDEO_CAMERA_SENSOR_SP0A19)
	return VGA;
	#else
	return SXGA;
	#endif
	...
	#elif defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_ARCH_TCC893X)
	{
		dprintk("[TCC892x] gpio init start!\n");
			...
			// Power doesn't need it.
				...
				else if(system_rev == 0x5000 || system_rev == 0x5001) { 				// M805_TCC8935
				...
				if(system_rev == 0x1005 || system_rev == 0x1007) {
					...
					if(CameraID)
					{
						#if defined(CONFIG_VIDEO_CAMERA_SENSOR_MT9M113) || defined(CONFIG_VIDEO_CAMERA_SENSOR_SR130PC10)  //2012.11.20 swhwang test
						gpio_request(TCC_GPF(15), NULL);
						gpio_direction_output(TCC_GPF(15), 1);
						#endif
					}
					...
					if(CameraID)
					{
						#if defined(CONFIG_VIDEO_CAMERA_SENSOR_MT9M113) || defined(CONFIG_VIDEO_CAMERA_SENSOR_SR130PC10) 
						gpio_request(TCC_GPD(2), NULL);
						gpio_direction_output(TCC_GPD(2), 1);
						#endif
					}
					...
					#else
						#if defined(CONFIG_VIDEO_CAMERA_SENSOR_MT9P111) || defined(CONFIG_VIDEO_CAMERA_SENSOR_MT9T111) || defined(CONFIG_VIDEO_CAMERA_SENSOR_S5K5CAGA)  //2012.11.20 swhwang test
							gpio_request(TCC_GPF(13), NULL);
							gpio_direction_output(TCC_GPF(13), 0);
						#else
							gpio_request(TCC_GPF(14), NULL);
			...
			// todo : 
				...
				if(system_rev == 0x1005 || system_rev == 0x1007){
						...
						else
						{
							...
							return gpio_get_value(TCC_GPF(30)); // must enable to use 5M(mt9p111)
						#else
							#if defined(CONFIG_VIDEO_CAMERA_SENSOR_MT9T113) || defined(CONFIG_VIDEO_CAMERA_SENSOR_SR130PC10) 
								return gpio_get_value(TCC_GPF(15));					
							#else
