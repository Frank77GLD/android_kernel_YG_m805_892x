...
#if defined(CONFIG_ARCH_TCC92XX) || defined(CONFIG_ARCH_TCC93XX)  || defined(CONFIG_ARCH_TCC88XX) || defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_ARCH_TCC893X)
#include <mach/bsp.h>
...
#define CAMERA_LOOP_LIMIT_COUNT 	1000 //20120425 ysseung   infinite loop limit is 10sec. -> 10ms * CAMERA_LOOP_LIMIT_COUNT
...
void cif_scaler_calc(void)
{
	...
	dprintk("%s():  src_x=%d, src_y=%d, off_x=%d, off_y=%d, zoom_step=%d. \n", __FUNCTION__, data->cif_cfg.main_set.source_x, \
			data->cif_cfg.main_set.source_y, off_x, off_y, data->cif_cfg.zoom_step);
	dprintk("%s():  scaler_x=%d, scaler_y=%d, scaler_off_x=%d, scaler_off_y=%d, tgt_x=%d, tgt_y=%d. \n", __FUNCTION__, \
			data->cif_cfg.main_set.scaler_x, data->cif_cfg.main_set.scaler_y, data->cif_cfg.main_set.win_hor_ofst, \
			data->cif_cfg.main_set.win_ver_ofst, data->cif_cfg.main_set.target_x, data->cif_cfg.main_set.target_y);
		...
		#elif defined(CONFIG_ARCH_TCC893X)
			VIOC_VIN_SetImageSize(pVINBase, width, height);
			VIOC_VIN_SetImageOffset(pVINBase, 0, 0, 0);
			VIOC_VIN_SetImageCropSize(pVINBase, data->cif_cfg.main_set.source_x, data->cif_cfg.main_set.source_y);
			VIOC_VIN_SetImageCropOffset(pVINBase, data->cif_cfg.main_set.win_hor_ofst, data->cif_cfg.main_set.win_ver_ofst);
			dprintk("VIN WxH[%dx%d] Crop Start XxY[%dx%d] \n", data->cif_cfg.main_set.source_x, data->cif_cfg.main_set.source_y, 	\
															data->cif_cfg.main_set.win_hor_ofst, data->cif_cfg.main_set.win_ver_ofst);
		#else
...
{
	uint *pLUT;
	uint width, height, offs_width, offs_height, offs_height_intl = 0;
	struct TCCxxxCIF *data = (struct TCCxxxCIF *)&hardware_data;
...
#else
	#if defined(CONFIG_MACH_M805_892X) || defined(CONFIG_MACH_M805_893X)  // Because of DE signal
	...
	VIOC_VIN_SetImageCropSize(pVIN, data->cif_cfg.main_set.source_x, data->cif_cfg.main_set.source_y);
	VIOC_VIN_SetImageCropOffset(pVIN, data->cif_cfg.main_set.win_hor_ofst, data->cif_cfg.main_set.win_ver_ofst);
#else
...
int tccxxx_vioc_scaler_set(VIOC_SC *pSC, VIOC_VIN *pVIN, VIOC_WMIX *pWMIX, uint plug_in)
{
	...
	VIOC_SC_SetBypass(pSC, OFF); 	// scaling mode.
	#if defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150)
		VIOC_SC_SetDstSize(pSC, mw, mh); // set destination size in scaler
		VIOC_SC_SetOutPosition(pSC, (mw-dw), (mh-dh));
	#else
		#if defined(CONFIG_ARCH_TCC893X)
			VIOC_SC_SetDstSize(pSC, (dw + 2), (dh + 2)); // set destination size in scaler
			VIOC_SC_SetOutPosition(pSC, 1, 1);
		#else
...
#elif defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_ARCH_TCC893X)
	gZoomStep = data->cif_cfg.zoom_step;
	data->cif_cfg.zoom_step = 0;
...
int tccxxx_cif_capture(int quality)
{
	...
	sc_plug_in0 = VIOC_SC_VIN_00; // VIOC_SC_WDMA_05;
	data->cif_cfg.oper_mode = OPER_CAPTURE;
	data->cif_cfg.zoom_step = gZoomStep;
	dprintk("%s() -  Zoom Step is %d. \n", __func__, data->cif_cfg.zoom_step);
	memset(&(data->cif_cfg.jpg_info), 0x00, sizeof(TCCXXX_JPEG_ENC_DATA));
...
int tccxxx_cif_open(void)
{
	...
	if(!cam_irq) {
		...
		#elif defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_ARCH_TCC893X)
			if((ret = request_irq(INT_VIOC_WD5, cif_cam_isr_in8920, IRQF_SHARED, "camera", data)) < 0)
		#else
			if((ret = request_irq(IRQ_CAM, cif_cam_isr, IRQF_DISABLED, "camera", NULL)) < 0)
		#endif
		{
			printk("FAILED to aquire camera-irq(%d).\n", ret);
			return ret;
		}
		cam_irq = 1;
	}
