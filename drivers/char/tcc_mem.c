...
extern  void tcc_video_skip_one_frame(int frame_id);
...
#elif defined(CONFIG_ARCH_TCC892X) && defined(CONFIG_DRAM_16BIT_USED)
	else
	{
		return 1;
	}
...
static int curr_displayed_idx = -1;
long tmem_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case TCC_VIDEO_SET_DISPLAYED_IDX:
		{
			curr_displayed_idx = (int)arg;
		}
		break;

		case TCC_VIDEO_GET_DISPLAYED_IDX:
		{
			return curr_displayed_idx;
		}
		break;
#endif
		...
		case TCC_LIMIT_PRESENTATION_WINDOW:
		{
			...
			if( res_region > PRESENTATION_LIMIT_RESOLUTION)
				ret = 1;
#endif
#endif
		}
