...
void hdmi_phy_reset_for_vioc(void);
...
int hdmi_set_color_space(enum ColorSpace space)
{
    ...
    int ret = 1;
	unsigned int iPXWD, iR2YMD, iR2Y, iSWAP;
...
int hdmi_set_audio_packet_type(enum HDMIASPType packet)
{
	...
	switch (packet)
	{
		...
		case HDMI_DSD:
			reg |= ASP_DSD_TYPE;
			break;
		// in case of HBR, set layout 1/SP0~3
		case HDMI_HBR:
			reg |= ASP_HBR_TYPE;
...
void hdmi_phy_reset(void)
{
	...
	if(phy_status)
	{
		dprintk(KERN_INFO "%s phy is ready : 10us * %d\n", __FUNCTION__, phy_chk_cnt);
		dprintk(KERN_INFO "%s Phy status = %d, Phy stats CMU = %d, Phy status PLL = %d\n", __func__, phy_status, phy_status_cmu, phy_status_pll);
	}
	else
	{
		dprintk(KERN_INFO "%s Phy status = %d, Phy stats CMU = %d, Phy status PLL = %d\n", __func__, phy_status, phy_status_cmu, phy_status_pll);
		dprintk(KERN_INFO "%s try phy reset again \n", __FUNCTION__);
		...
		{
			dprintk(KERN_INFO "%s phy is not ready\n", __FUNCTION__);
			dprintk(KERN_INFO "%s Phy status = %d, Phy stats CMU = %d, Phy status PLL = %d\n", __func__, phy_status, phy_status_cmu, phy_status_pll);
		}
	}
}
...
void hdmi_phy_reset_for_vioc(void)
{
	...
	unsigned int phy_status_cmu;
	unsigned int phy_status_pll;
	unsigned int phy_chk_cnt = 0, phy_try_cnt = 0;
