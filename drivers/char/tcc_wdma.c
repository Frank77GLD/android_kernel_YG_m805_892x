...
volatile PVIOC_RDMA 	pWDMA_disp_base;
...
#define WDMA_DEV_MAJOR		0

static int	wdma_major = WDMA_DEV_MAJOR;

static unsigned int WDMA_BaseAddr;
static struct clk *wdma_clk;
static pmap_t fb_screen_pbuf0;
char Fb_Lcdc_Num = 0;
...
char tccxxx_wdma_ctrl(unsigned int argp)
{
	...
	VIOC_WDMA_IMAGE_INFO_Type ImageCfg;
	 ...
	 unsigned int r2yEn = 1;	//y2r
       memset((char *)&ImageCfg, NULL, sizeof(ImageCfg));

	unsigned int base_addr;
	static unsigned int Screen_yoffSet = 0;
	...
	if(copy_from_user( &ImageCfg, (VIOC_WDMA_IMAGE_INFO_Type *)argp, sizeof(ImageCfg))){
	  ...
	  ImageCfg.TargetWidth = ImageCfg.ImgSizeWidth;		//test
      ImageCfg.TargetHeight = ImageCfg.ImgSizeHeight;		//test

	  ImageCfg.ImgFormat = TCC_LCDC_IMG_FMT_YUV420SP;


	  /* scaler setting */
	VIOC_SC_SetSrcSize(pWDMA_scaler, ImageCfg.ImgSizeWidth, ImageCfg.ImgSizeHeight);
	...
	//base_addr = (unsigned int )fb_screen_pbuf0.base + (ImageCfg.ImgSizeWidth * ImageCfg.ImgSizeWidth * 3/2 * Screen_yoffSet);//test
	...
	//printk(" w:%d h:%d base:0x%08x Screen_yoffSet=%d \n",ImageCfg.ImgSizeWidth,ImageCfg.ImgSizeHeight,ImageCfg.BaseAddress,Screen_yoffSet);
 	//printk(" base1:0x%08x  base2:0x%08x  \n",ImageCfg.BaseAddress1, ImageCfg.BaseAddress2);
	...
	else
	{
       ...
       VIOC_WDMA_SetWifiDisplayImageConfig(pWDMA_wdma_base, &ImageCfg, r2yEn);
...
tccxxx_wdma_cleanup(void)
{
	...
	unregister_chrdev(dev, MINOR(dev));
	clk_put(wdma_clk);
	return;
}
