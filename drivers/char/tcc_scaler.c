...
char M2M_Scaler_Ctrl_Detail(SCALER_TYPE *scale_img)
{
	...
	dprintk("Src:  addr:0x%x, 0x%x, 0x%x,  fmt:%d. \n", (unsigned int)scale_img->src_Yaddr, 		\
		(unsigned int)scale_img->src_Uaddr, (unsigned int)scale_img->src_Vaddr, scale_img->src_fmt);
	dprintk("Dst:  addr:0x%x, 0x%x, 0x%x,  fmt:%d. \n", (unsigned int)scale_img->dest_Yaddr, 		\
		(unsigned int)scale_img->dest_Uaddr, (unsigned int)scale_img->dest_Vaddr, scale_img->dest_fmt);
	dprintk("Size : src_w:%d, src_h:%d, dst_w:%d, dst_h:%d. \n", scale_img->src_ImgWidth, 		\
		scale_img->src_ImgHeight, scale_img->dest_ImgWidth, scale_img->dest_ImgHeight);
...
#if defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_ARCH_TCC893X)
extern void TCC_VIQE_DI_PlugInOut_forAlphablending(int plugIn);
char Alphablending_Ctrl_Func(SCALER_ALPHABLENDING_TYPE* apb_info)
{
	char ret = 0;
	VIOC_SCALER_INFO_Type pScalerInfo;
	int CheckVIQEPlugInOut, CheckDEINTSPlugInOut;
	unsigned int nVIQEPath, nDEINTSPath;

	spin_lock_irq(&(sc_data.cmd_lock));
	// Set to VIOC_VIQE or VIOC_DEINTLS
	CheckVIQEPlugInOut = VIOC_CONFIG_CheckPlugInOut(VIOC_VIQE, &nVIQEPath);
	CheckDEINTSPlugInOut = VIOC_CONFIG_CheckPlugInOut(VIOC_DEINTLS, &nDEINTSPath);
	if((CheckVIQEPlugInOut == VIOC_PATH_CONNECTED && nVIQEPath==8 ) 
		|| (CheckDEINTSPlugInOut == VIOC_PATH_CONNECTED &&nDEINTSPath==8) ) /*RDMA12*/
	{
		//Plug out
		//TCC_VIQE_DI_PlugInOut_forAlphablending(0); //20130220 ysseung   사용 유무 파악이 안되어 임시로 막음.
	}
	...
	{
		//Plug in
		//TCC_VIQE_DI_PlugInOut_forAlphablending(1); //20130220 ysseung   사용 유무 파악이 안되어 임시로 막음.
	}
	...
	switch(cmd)
	{
		...
		case TCC_SCALER_IOCTRL_KERENL:
			...
			if(msc_data->block_operating) {
				...
				if(ret <= 0) {
					msc_data->block_operating = 0;
					printk("%s(%d):  scaler 0 timed_out block_operation:%d!! cmd_count:%d \n", __func__, ret, msc_data->block_waiting, msc_data->cmd_count);
				}
				ret = 0;
			}
