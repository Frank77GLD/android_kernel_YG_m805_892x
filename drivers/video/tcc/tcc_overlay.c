...
{
	tccxxx_overlay_set_pos(buffer_cfg.cfg);
	tccxxx_overlay_q_buffer(buffer_cfg.addr);
...
static long tccxxx_overlay_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		case OVERLAY_SET_IGNORE_PRIORITY:
			return tccxxx_overlay_ignore_priority((unsigned int*)arg);
		...
		case OVERLAY_SET_POSITION:
			...
			return tccxxx_overlay_set_pos(overCfg);

		case OVERLAY_QUEUE_BUFFER:
			if(copy_from_user(&curY_phyAddr, (unsigned int *)arg, sizeof(unsigned int)))
				return -EFAULT;

			return tccxxx_overlay_q_buffer(curY_phyAddr);

		case OVERLAY_SET_CONFIGURE:
			if(copy_from_user(&overCfg, (overlay_config_t *)arg, sizeof(overlay_config_t)))
				return -EFAULT;

			return tccxxx_overlay_set_configure(overCfg);
...
static int __init tcc_overlay_probe(struct platform_device *pdev)
{
	...
	BUG_ON(overlay_lcdc_clk == NULL);
		...
		//TCC893X LCD VIDEO RDMA(7) WMIXER & DISP(0)
