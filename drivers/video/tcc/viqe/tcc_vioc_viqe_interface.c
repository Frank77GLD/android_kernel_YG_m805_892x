...
#if defined(CONFIG_MACH_TCC8920) || defined(CONFIG_SOC_TCC8925S) || defined(CONFIG_ARCH_TCC893X)
						...
						int crop_top, int crop_bottom, int crop_left, int crop_right, int OddFirst)
{
	...
	else
	{
		...
		else if(scalerCh == 1)
		{
			...
			#else
				gRDMA_reg_30Hz = HwVIOC_RDMA02;
				gVIQE_RDMA_num_30Hz = VIOC_VIQE_RDMA_02;
			#endif
		}
		else
		{
						...
						unsigned int offset_x, unsigned int offset_y, int OddFirst)
{
	...
	if(gOutputMode == TCC_OUTPUT_HDMI)
	{
		#if defined(CONFIG_ARCH_TCC893X)
