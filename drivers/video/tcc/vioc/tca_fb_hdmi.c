...
void TCC_HDMI_SET_OnTheFly(char hdmi_lcdc, struct tcc_lcdc_image_update *ImageInfo)
{
	...
	{
		printk(" %s  MVC frame just return \n",__func__);
		return;
	}
...
void TCC_LCD_DISPLAY_UPDATE(char hdmi_lcdc, struct tcc_lcdc_image_update *ImageInfo)
{
	...
	if((!lcd_width) || (!lcd_height)){
		printk("LCD :: Error :: lcd_width %d, lcd_height %d \n", lcd_width, lcd_height);
		return;
	}
...
void TCC_HDMI_DISPLAY_UPDATE(char hdmi_lcdc, struct tcc_lcdc_image_update *ImageInfo)
{
	...
	if((!lcd_width) || (!lcd_height)){
		printk("Error :: lcd_width %d, lcd_height %d \n", lcd_width, lcd_height);
		return;
	}
...
void TCC_HDMI_DISPLAY_UPDATE_3D(char hdmi_lcdc, struct tcc_lcdc_image_update *ImageInfo)
{
	...
	else
	{
		pDISPBase = (VIOC_DISP*)tcc_p2v(HwVIOC_DISP0);
		pWMIXBase = (VIOC_WMIX*)tcc_p2v(HwVIOC_WMIX0);
		pRDMABase0 = (VIOC_RDMA*)tcc_p2v(HwVIOC_RDMA03);
		pRDMABase0 = (VIOC_RDMA*)tcc_p2v(HwVIOC_RDMA04);
	}
	...
	//
	// add checking code for 720p frame packing
	//

	ImageInfo->Image_width = 1280;
	ImageInfo->Image_height = 720;

	//ImageInfo->Frame_width = 1280;
	//ImageInfo->Frame_height = 720;

	if( (ImageInfo->Image_width > 1280 || ImageInfo->Image_height > 720))
	{
		printk("%s - image width and height is not support\n", __func__);
		return;	
	}
	...
	VIOC_WMIX_SetPosition(pWMIXBase, ImageInfo->Lcdc_layer, 0, 0);
	VIOC_RDMA_SetImageScale(pRDMABase0, 0, 0);
	...
	// position
	VIOC_WMIX_SetPosition(pWMIXBase, ImageInfo->Lcdc_layer, 0, 720 + 30);

	// scale
	VIOC_RDMA_SetImageScale(pRDMABase1, 0, 0);
