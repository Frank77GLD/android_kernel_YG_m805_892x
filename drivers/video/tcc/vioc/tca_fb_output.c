...
static irqreturn_t TCC_OUTPUT_LCDC_Handler(int irq, void *dev_id)
{
...
#ifdef CONFIG_VIOC_FIFO_UNDER_RUN_COMPENSATION
	if(LCDCstatus & VIOC_DISP_IREQ_FU_MASK)
	{
		...
		VIOCReset++;
		dprintk("%s VIOC_DISP_IREQ_FU_MASK :%d STATUS:0x%x LCDC_NUM:%d \n",__func__,VIOCReset, LCDCstatus, pDISP_OUTPUT[OutputType].LCDC_N);				
	}

	if(LCDCstatus & VIOC_DISP_IREQ_DD_MASK)
	{
		...
		if(VIOCReset)		{
			volatile static  VIOC_DISP pDISPBackup;
			VIOC_DISP_TurnOff(pDISP_OUTPUT[OutputType].pVIOC_DispBase);

			pDISPBackup = *pDISP_OUTPUT[OutputType].pVIOC_DispBase;

			// reset vioc display path
			VIOC_DISP_SWReset(pDISP_OUTPUT[OutputType].LCDC_N);
			
			*pDISP_OUTPUT[OutputType].pVIOC_DispBase = pDISPBackup;
	...
	if(LCDCstatus & VIOC_DISP_IREQ_RU_MASK)
	{
...
void TCC_OUTPUT_LCDC_Init(void)
{
	...
	pmap_get_info("ovelay1", &pmap);
	fb_g2d_pbuf0 = (char *) pmap.base;
	fb_g2d_pbuf1 = (char *) pmap.base + pmap.size/2;
	#else
	...
	#if defined(TCC_OUTPUT_STARTER_DUAL) || defined(TCC_OUTPUT_STARTER_DUAL_AUTO)
	...
	else 
	{
		if(output_lcdc_onoff == 1)
		{
			if( vioc_scaler_plug_in )
			{
					...
					{
						if(lcdc_num)
							VIOC_SC_SetSWReset(VIOC_SC2, 4, 0xFF);
						else
							VIOC_SC_SetSWReset(VIOC_SC2, 0, 0xFF);
...
void TCC_OUTPUT_LCDC_OnOff(char output_type, char output_lcdc_num, char onoff)
{
	...
	if(onoff)
	{
		...
		if(output_lcdc_num) 
		{
			...
			pDISP_OUTPUT[output_type].pVIOC_RDMA_Mouse = 0;
			#elif defined(CONFIG_ARCH_TCC893X)
			pDISP_OUTPUT[output_type].pVIOC_RDMA_Video = (VIOC_RDMA*)tcc_p2v(HwVIOC_RDMA07);
		...
		if(pDISP_OUTPUT[output_type].pVIOC_RDMA_Mouse)
...
char TCC_OUTPUT_FB_Process(unsigned int width, unsigned int height, unsigned int bits_per_pixel, unsigned int addr, unsigned int type)
{
	...
	// alpha & chroma key color setting
		...
		#if defined(CONFIG_ARCH_TCC893X)
			VIOC_WMIX_SetChromaKey(pDISP_OUTPUT[type].pVIOC_WMIXBase, 2, 1, 0x00, 0x80, 0x80, 0xFF, 0xFF, 0xFF);			
		#else
...
char TCC_OUTPUT_FB_Update_3D(unsigned int width, unsigned int height, unsigned int bits_per_pixel, unsigned int addr, unsigned int type)
{
	...
	else
	{
		...
		//VIOC_WMIX_SetPosition(pDISP_OUTPUT[type].pVIOC_WMIXBase, 0, lcd_w_pos, lcd_h_pos);
		VIOC_WMIX_SetPosition(pDISP_OUTPUT[type].pVIOC_WMIXBase, 1, 0, 720+30);
...
void TCC_OUTPUT_FB_UpdateSync(unsigned int type)
{
	...
	#if defined(CONFIG_TCC_OUTPUT_ATTACH) || defined(TCC_OUTPUT_STARTER_DUAL) || defined(TCC_OUTPUT_STARTER_DUAL_AUTO)
