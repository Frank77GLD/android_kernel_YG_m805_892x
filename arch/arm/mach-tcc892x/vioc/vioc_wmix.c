...
void VIOC_WMIX_SetChromaKey(VIOC_WMIX *pWMIX, unsigned int nLayer, unsigned int nKeyEn, unsigned int nKeyR, unsigned int nKeyG, unsigned int nKeyB, unsigned int nKeyMaskR, unsigned int nKeyMaskG, unsigned int nKeyMaskB)
{
	switch (nLayer)
	{
		case 0 :    // top layer
			...
			*/
			BITCSET(pWMIX->uKEY0.nREG[0], 0x80FFFFFF, (nKeyEn << 31) | (nKeyR << 16) | (nKeyG << 8) | nKeyB );
		...
		case 1 :
			...
			*/
			BITCSET(pWMIX->uKEY1.nREG[0], 0x80FFFFFF, (nKeyEn << 31) | (nKeyR << 16) | (nKeyG << 8) | nKeyB );
		...
		case 2 :
			...
			*/
			BITCSET(pWMIX->uKEY2.nREG[0], 0x80FFFFFF, (nKeyEn << 31) | (nKeyR << 16) | (nKeyG << 8) | nKeyB );
