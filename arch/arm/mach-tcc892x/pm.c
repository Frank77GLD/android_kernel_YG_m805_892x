...
//	*(volatile unsigned long *)addr_iocfg(0x18) &= ~Hw29;		// I2C_0 S/W RESET
	...
	*(volatile unsigned long *)addr_iocfg(0x18) |= Hw29;		// I2C_0 S/W RESET
	...
	if (PMIC_PARAM(DEV_CH) == 0) {
		...
		if (PMIC_PARAM(SYSTEM_REV) == 0x2002 || PMIC_PARAM(SYSTEM_REV) == 0x2003 || PMIC_PARAM(SYSTEM_REV) == 0x2004 || PMIC_PARAM(SYSTEM_REV) == 0x2005 || PMIC_PARAM(SYSTEM_REV) == 0x2006 || PMIC_PARAM(SYSTEM_REV) == 0x2007 || PMIC_PARAM(SYSTEM_REV) == 0x2008 || PMIC_PARAM(SYSTEM_REV) == 0x2009) {
			//I2C[12] - GPIOC[2][3]
...
//Bruce, wake-up source should be surely set here !!!!
	...
	{
		if(*(volatile unsigned long *)SRAM_STACK_ADDR == 0x2006 || *(volatile unsigned long *)SRAM_STACK_ADDR == 0x2007 || *(volatile unsigned long *)SRAM_STACK_ADDR == 0x2008 || *(volatile unsigned long *)SRAM_STACK_ADDR == 0x2009){
			//set wake-up polarity
