...
#define BT_WAKE_GPIO TCC_GPC(25) //for bcm4330
#define BT_HOST_WAKE_GPIO TCC_GPADC(4) //for bcm4330
#define EXT_IRQ 14 //for bcm4330
...
extern int tcc_uart_enable(int port_num );
extern int tcc_uart_disable(int port_num );
...
static int bcm_bt_lpm_init(struct platform_device *pdev)
{
	...
	irq = EXT_IRQ; // EXT INT 12
	tcc_gpio_config_ext_intr(irq, EXTINT_GPIOC_25);
...
#else
	gpio_direction_output(BT_WAKE_GPIO, 1);
#endif
