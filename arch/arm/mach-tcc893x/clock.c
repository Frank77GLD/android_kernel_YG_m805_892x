/*
 * linux/arch/arm/mach-tcc893x/clock.c
 *
 * Clock driver for tcc893x
 *
...
static unsigned long private_timer_get_rate(struct clk *clk)
{
    unsigned long flags;
    ...
    return (unsigned long)(clk->real_rate/3);
}

static int private_timer_enable(struct clk *clk)
{
    return 0;
}
static void private_timer_disable(struct clk *clk)
{
    return;
}
...
static int max_io_enable(struct clk *clk)
{
    unsigned long flags;
    dbg("%s: clk:%s\n", __func__, clk->name);
    local_irq_save(flags);

	tca_ckc_setfbusctrl(FBUS_IO  , NOCHANGE, 1800000);
	tca_ckc_setfbusctrl(FBUS_HSIO, NOCHANGE, 2500000);

    local_irq_restore(flags);
    return 0;
}

static void max_io_disable(struct clk *clk)
{
    unsigned long flags;
    dbg("%s: clk:%s\n", __func__, clk->name);
    local_irq_save(flags);

	tca_ckc_setfbusctrl(FBUS_IO  , NOCHANGE, 1333334);
	tca_ckc_setfbusctrl(FBUS_HSIO, NOCHANGE, 1250000);

    local_irq_restore(flags);
}

static struct clk clk_max_io = {
    .name           = "max_io_clk",
    .flags          = CLK_AUTO_OFF,
    .enable         = max_io_enable,
    .disable        = max_io_disable,
};
#endif
...
static struct clk clk_timerp = {
    .name           = "smp_twd",
    .flags          = CLK_ALWAYS_ENABLED,
    .enable         = private_timer_enable,
    .disable        = private_timer_disable,
    .get_rate       = private_timer_get_rate,
    .clock_idx      = FBUS_CPU,
};
...
static struct clk clk_g2d = {
    ...
    .pwdn_idx       = FBUS_G2D,
    .pwdn           = tca_ckc_setfbuspwdn,
    .swreset_idx    = FBUS_G2D,
    .swreset        = tca_ckc_setfbusswreset,
    .clock_idx      = FBUS_G2D,
    .clock          = tca_ckc_fclk_enable,
};

static struct clk clk_cm3 = {
    .name           = "cortex-m3",
    .flags          = CLK_AUTO_OFF,
    .enable         = fclk_enable,
    .disable        = fclk_disable,
...
static struct clk clk_pktgen0 = {
    ...
    .set_rate       = pclk_set_rate,
    .get_rate       = pclk_get_rate,
    .clock_idx      = PERI_PKTGEN0,
    .clock          = tca_ckc_pclk_enable,
};

static struct clk clk_pktgen1 = {
    .name           = "pkt_gen1",
    .flags          = CLK_AUTO_OFF,
    .enable         = pclk_enable,
    .disable        = pclk_disable,
    .set_rate       = pclk_set_rate,
    .get_rate       = pclk_get_rate,
    .clock_idx      = PERI_PKTGEN1,
    .clock          = tca_ckc_pclk_enable,
};

static struct clk clk_pktgen2 = {
    .name           = "pkt_gen2",
    .flags          = CLK_AUTO_OFF,
    .enable         = pclk_enable,
    .disable        = pclk_disable,
    .set_rate       = pclk_set_rate,
    .get_rate       = pclk_get_rate,
    .clock_idx      = PERI_PKTGEN2,
    .clock          = tca_ckc_pclk_enable,
};

static struct clk clk_pktgen3 = {
    .name           = "pkt_gen3",
    .flags          = CLK_AUTO_OFF,
    .enable         = pclk_enable,
    .disable        = pclk_disable,
...
static struct clk clk_hsioaxibus = {
    .name           = "hsioaxibus",
    .parent         = &clk_hsio,
    .flags          = CLK_AUTO_OFF,
    .enable         = pclk_enable,
...
//    &clk_jpeg,
    ...
    &clk_otg,           //          OTG (USB3.0)
    &clk_hsioaxibus,    // bit 16   HSIO CFG
    &clk_hsio,
...
struct clk *clk_get_sys(const char *dev_id, const char *con_id)
{
    ...
    mutex_lock(&clock_mutex);
    list_for_each_entry(clk, &clock_list, list) {
        if (!strcmp(dev_id, clk->name))
            goto found;
        if (!strcmp(dev_id, clk->name) && clk->dev == NULL)
            goto found;
    }
    clk = ERR_PTR(-ENOENT);
found:
...
static int tcc_clk_show(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "memory bus  : %4d MHz\n", (tca_ckc_getfbusctrl(FBUS_MEM)*2)/10000);
    ...
    seq_printf(m, "g2d bus     : %4d MHz\n", tca_ckc_getfbusctrl(FBUS_G2D)/10000);
    seq_printf(m, "Cortex-M3   : %4d MHz\n", tca_ckc_getfbusctrl(FBUS_CM3)/10000);
