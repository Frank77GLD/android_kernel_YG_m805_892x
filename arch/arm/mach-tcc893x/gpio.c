...
static int tcc893x_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	gpioregs *reg = tcc893x_gpio_to_reg(chip->base);
...
static void tcc893x_gpio_set(struct gpio_chip *chip, unsigned offset, int value)
{
	gpioregs *reg = tcc893x_gpio_to_reg(chip->base);
	unsigned long flags;
...
static int tcc893x_gpio_direction_input(struct gpio_chip *chip, unsigned offset)
{
	gpioregs *reg = tcc893x_gpio_to_reg(chip->base);
	unsigned long flags;
...
static int tcc893x_gpio_direction_output(struct gpio_chip *chip, unsigned offset, int value)
{
	gpioregs *reg = tcc893x_gpio_to_reg(chip->base);
	unsigned long flags;
...
static struct gpio_chip tcc893x_gpio_chips[] = {
	{
		.label = "gpio_a",
		.base = GPIO_PORTA,
		.ngpio = 32,
...
int __init tcc893x_gpio_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(tcc893x_gpio_chips); i++) {
		chip = &tcc893x_gpio_chips[i];

		chip->direction_input = tcc893x_gpio_direction_input;
		chip->direction_output = tcc893x_gpio_direction_output;
		chip->set = tcc893x_gpio_set;
		chip->get = tcc893x_gpio_get;
...
postcore_initcall(tcc893x_gpio_init);
