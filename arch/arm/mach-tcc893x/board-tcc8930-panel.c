/* linux/arch/arm/mach-tcc893x/board-tcc8930-panel.c
 *
...
#ifdef CONFIG_LCD_LMS700KF23
static struct platform_device lms700kf23_lcd = {
	.name	= "lms700kf23_lcd",
	.dev	= {
		.platform_data	= &lcd_pdata,
	},
...
static void tcc8930_brightness_set(struct led_classdev *led_cdev, enum led_brightness value)
{
...
static struct led_classdev tcc8930_backlight_led = {
	.name		= "lcd-backlight",
	.brightness	= DEFAULT_BACKLIGHT_BRIGHTNESS,
	.brightness_set	= tcc8930_brightness_set,
};

static int tcc8930_backlight_probe(struct platform_device *pdev)
{
	...
	return led_classdev_register(&pdev->dev, &tcc8930_backlight_led);
}

static int tcc8930_backlight_remove(struct platform_device *pdev)
{
	led_classdev_unregister(&tcc8930_backlight_led);
	return 0;
}

static struct platform_driver tcc8930_backlight_driver = {
	.probe	= tcc8930_backlight_probe,
	.remove	= tcc8930_backlight_remove,
	.driver	= {
		.name	= "tcc8930-backlight",
		.owner	= THIS_MODULE,
	},
};

static struct platform_device tcc8930_backlight = {
	.name = "tcc8930-backlight",
};
...
int __init tcc8930_init_panel(void)
{
...
#ifdef CONFIG_LCD_LMS700KF23
	case PANEL_ID_LMS700KF23:
		platform_device_register(&lms700kf23_lcd);
		break;
#endif//
	...
	platform_device_register(&tcc8930_backlight);
	ret = platform_driver_register(&tcc8930_backlight_driver);
	if (ret)
		return ret;
