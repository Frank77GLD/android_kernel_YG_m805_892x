...
#if !defined(CONFIG_SND_TCC_DAI2SPDIF)
#if defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_ARCH_TCC893X) || defined(CONFIG_SOC_TCC8925S) // Planet 20121025 SPDIF exists only on channel01
    {
        .name = "TCC",
        .stream_name = "IEC958",
...
static int __init tcc_init_wm8524(void)
{
    ...
    if( !(machine_is_tcc8800st() || machine_is_tcc9300st() || machine_is_tcc8920st() || machine_is_tcc8930st()) ) {
        alsa_dbg("\n\n\n\n%s() do not execution....\n\n", __func__);
        return 0;
    }
